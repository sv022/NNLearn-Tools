...
<template>
    <div>
        <div>
            <div v-for="i in convRGBStore.outputG.height" v-bind:key="i" class="flex">
                <OutputPixel v-for="j in convRGBStore.outputG.height" v-bind:key="i * convRGBStore.outputG.height + j" channel="G"
                    :value="convRGBStore.outputG.pixels[((i - 1) * convRGBStore.outputR.width) + (j - 1)]" :size="pixelSize"
