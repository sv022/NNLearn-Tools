<script setup
    ...
    function getHighlightFrame(w: number, h: number) {
        ...
        visualsStore.getHighlightFrame(w, h, convRGBStore.kernel.width, convRGBStore.inputResultB, convRGBStore.padding, convRGBStore.stride)
    }
...
<template>
    <div>
        <div>
            <div v-for="i in convRGBStore.outputB.height" v-bind:key="i" class="flex">
                <OutputPixel v-for="j in convRGBStore.outputB.height" v-bind:key="i * convRGBStore.outputB .height + j" channel="B"
                    :value="convRGBStore.outputB.pixels[((i - 1) * convRGBStore.outputB.width) + (j - 1)]" :size="pixelSize"
