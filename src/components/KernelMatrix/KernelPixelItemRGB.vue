  ...
  <HoverCard>
    <div class="flex flex-col items-center">
      <HoverCardTrigger>
        <div
          :class="
            cn('flex justify-center items-center border text-gray-500', props.size, pixelFontSize)
          "
      ...
      <div class="flex items-center justify-center h-10 w-10 text-nowrap text-md">
        <mark>x <wbr /></mark
        ><mark :class="kernelValueTextStyle">{{ props.value.toFixed(2) }}</mark>
      </div>
    </div>
    <HoverCardContent>
      <div class="space-y-2">
        ...
        <span class="flex">
          <Button
            variant="ghost"
            @click="decrementKernelPixel(step)"
            @click.shift.stop="decrementKernelPixel(step * 10)"
            >-</Button
          >
          <Slider
            id="slider"
          ...
          <Button
            variant="ghost"
            @click="incrementKernelPixel(step)"
            @click.shift.stop="incrementKernelPixel(step * 10)"
            >-</Button
          >
        </span>
