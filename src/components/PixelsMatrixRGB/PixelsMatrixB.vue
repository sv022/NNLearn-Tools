<script setup
  ...
  const width = computed(() => {
      return convRGBStore.inputResultB.width
  })
  const height = computed(() => {
      return convRGBStore.inputResultB.height
  })
    ...
    <div>
        <div v-for="i in height" v-bind:key="-i" class="flex">
            <PixelItem v-for="j in width" v-bind:key="i * height + j"
                :value="convRGBStore.inputResultB.pixels[((i - 1) * width) + (j - 1)]" channel='B' :size="pixelSize"
